# Specification authoring instructions

How the drafts in this repository are written and modelled. [`CONTRIBUTING.md`](CONTRIBUTING.md)
covers the *process* — fork, branch, regenerate, validate, PR. This file covers the *style*:
the editorial and modelling conventions the drafts have converged on.

Most rules below exist because breaking them produced a real defect that a green validation
run did not catch. The reason is given with each one; when a rule and a reason disagree,
follow the reason.
...
truth for the NodeSet, the NodeId CSV and the Annex table. This is also in `CONTRIBUTING.md`; it
is repeated here because most of the rules below are consequences of it.
