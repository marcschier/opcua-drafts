...
Description per corpus fixture; each projects to the nodes clause 5.6 materializes, under the
comparison Annex F.6 describes — both sides of which are implemented here, so it establishes that
the annex's rules are self-consistent and complete for these fixtures rather than that any WoT
...
| `examples/wot/submodels/X.td.jsonld` | the same, plus `uav:id`, `uav:browseName`, `uav:componentOf` and the ordering links | projects to the AddressSpace clause 5.6 defines |

All three are generated by `tools/jsonld/build_examples.py` from the templates cached by
`dpp_inventory.py`, and none is written unless it passes its own check first: the authored form must
lower back to the published template, the minimal form must still carry the graph it was made from
