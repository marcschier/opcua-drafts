...
|---|---|---|---|---|
| ns=2;i=4001 | UsesModel | IsUsedByDeployment | NonHierarchicalReferences | Links a Deployment to the Model it executes. Clause 6.5 requires exactly one such reference per deployment so the model serving now is unambiguous. Historical result provenance uses the ModelUsed identity returned by invocation and retained by the consuming specification. |
...
| Digest | Variable | ByteString | Scalar | Mandatory | Cryptographic digest of the model artefact, for provenance and integrity. Mandatory: clause 12 requires it for every model whose artefact is obtainable through ArtifactUri, and it is the terminus of the historical provenance chain from ModelUsed. |
