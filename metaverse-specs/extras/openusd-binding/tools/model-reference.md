...
| ns=1;i=6007 | TargetNodeId | Variable | Optional direct NodeId of the target when it is an in-server materialized Variable (a Part 2 UsdAttributeType). When present it takes precedence over the TargetPrimPath/TargetPropertyName pair, which remain the portable descriptors. |
...
| ns=1;i=6034 | AlarmAspect | Variable | Which A&C condition aspect drives the target (ActiveState/Severity/AckedState/EnabledState). |
| ns=1;i=1009 | OpenUsdHistoryBindingType | ObjectType | Read-only history (Part 11 HistoryRead) authored as USD time samples. |
| ns=1;i=6035 | TimeSampled | Variable | Author values as USD time samples (playback) rather than the latest default. |
...
| ns=1;i=6036 | CommandTargetNodeId | Variable | The Variable to write, or the Object on which to Call CommandMethodId. |
| ns=1;i=6037 | CommandMethodId | Variable | Optional Method to invoke instead of a Variable write. |
| ns=1;i=6038 | CommandTriggerPropertyName | Variable | The USD attribute whose change is interpreted as the command intent/value. |
