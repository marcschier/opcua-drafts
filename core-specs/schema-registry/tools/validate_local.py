...
for tag, el in elems:
    ...
    if tag in ("UAVariable", "UAObject", "UAMethod") and el.get("ParentNodeId"):
        ...
        if "HasModellingRule" not in reftypes and not is_enc and not wellknown_parent:
            ...
            if not (parsed and parsed[1] in (62100, 62101)):
...
registry = next((el for tag, el in elems if el.get("NodeId") == "ns=1;i=62100"), None)
if registry is None:
    errors.append("SchemaRegistry well-known instance ns=1;i=62100 missing")
