...
KNOWN_BASE = {
    ...
    "i=11575",  # FileType (Part 5) — HasTypeDefinition target of OpenUsdAssetType.File
}
...
def main():
    ...
    if len(models) != 1:
        ...
        if not any(r.get("ModelUri") == "http://opcfoundation.org/UA/" for r in reqs):
    ...
    # (e.g. the well-known OpenUSD object under Server i=2253). Type members hang
    # off a Type node in this set and DO require a ModellingRule.
    def is_concrete_instance(node):
        ...
        while True:
            ...
            if not p or cur.get("NodeId") in seen:
                # no parent -> a type or top-level; treat as declaration only if a type
                return False
            seen.add(cur.get("NodeId"))
            if p not in by_id:
                return True  # roots at an external/base node -> concrete instance
            cur = by_id[p]
            if cur.tag[len(NS):] in ("UAObjectType", "UAVariableType", "UADataType",
                                     "UAReferenceType"):
                return False  # member of a type -> instance declaration
    ...
    for n in nodes:
        ...
        if n.get("ParentNodeId"):
            ...
            # must have HasTypeDefinition (variables/objects) unless it's a Method/enumstrings etc.
            ...
            # modelling rule required unless a documented constant or a concrete instance
            ...
            # ParentNodeId must be backed by an inverse hierarchical reference to it
    ...
    # enum EnumStrings array size == field count
    for n in nodes:
