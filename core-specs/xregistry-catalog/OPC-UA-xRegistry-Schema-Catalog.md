...
The companion *OPC UA — Schema Registry* NodeSet (`../schema-registry/`) is the **in‑server, AddressSpace‑native projection** of this same model: its `SchemaRegistry` → `SchemaGroup` → `Schema` → `SchemaVersion` ObjectTypes are isomorphic to the xRegistry `registry` → `schemagroups` → `schemas` → `versions` shape (§5), so an OPC UA REST GET of the `SchemaRegistry` subtree yields an xRegistry‑compatible JSON document and an imported xRegistry catalog can be projected into those ObjectTypes without loss of the `opcua.*` labels §6 relies on. It also adds a fast in‑server variant of the §6 step 0 resolution: each schema Version document is additionally addressable by an **Opaque NodeId whose Identifier is the raw on‑wire SchemaId bytes**, so a decoder holding only a message's SchemaId performs a single `Read` (or a `GetSchema(SchemaId)` Method call) to obtain the document with no Browse and no fingerprint recomputation. A Server that hosts both the out‑of‑band xRegistry catalog and the in‑server registry keeps them consistent; the in‑server registry may operate authoritatively or as a TTL‑cached mirror of the external xRegistry.
