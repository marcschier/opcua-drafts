...
| NodeId | `record NodeId` | Fields: namespace Int32, idType enum/int, and exactly one identifier member: numeric UInt32-as-long, string, guid fixed16, or opaque bytes. A field MAY instead be the textual `string` form of §5.2.1. |
| ExpandedNodeId | `record ExpandedNodeId` | Fields: NodeId, nullable namespaceUri, serverIndex UInt32-as-long. A field MAY instead be the textual `string` form of §5.2.1. |
...
3. **Keep the opaque fallback at a fixed index.** Place the `"bytes"` fallback once and never move it: in the deterministic bounded schema of §6 it follows the initial known-struct branches (`["null", <known structs…>, "bytes"]`), and a field that started unbounded keeps it as the first non-null branch (`["null", "bytes", …]`). Because growth only ever **appends** new known-struct branches at the end of the union (step 2), the `"bytes"` index — and every earlier branch index — stays fixed, so an opaque body written under an earlier minor still decodes as `"bytes"`.
