...
A publisher **shall** advance MinorVersion when it aggregates a new Variant body form or ExtensionObject concrete type into the schema, and shall publish the resulting schema — registry registration plus announcement — per §5.2.3–5.2.6. The per-encoding procedure for narrowing and append-only growing these unions is in the Arrow Part 6 DataEncoding §5.6.12.
