#!/usr/bin/env python3
"""Worked example for §5.13 (binding inheritance & facet composition).
...
BINDNS = "http://opcfoundation.org/UA/PubSubBinding/"          # provisional base-spec ns
...
class Emit:
    ...
    def document(self):
        ...
        header = ['<?xml version="1.0" encoding="utf-8"?>',
                  '<!-- Facets: illustrative overlay for the PubSub Scenario Binding '
...
def emit_addendum(db):
    L = []
    A = L.append
    A("# OPC UA — PubSub Scenario Binding — Facets & Inheritance Addendum")
    A("")
    A("*Non-normative. Companion to the base specification, section “Binding inheritance and "
      "facet composition” (§5.13). Generated by `build_facets_example.py` from the self-contained "
    ...
    A("`Opc.Ua.Facets.ScenarioBinding.NodeSet2.xml` in this folder shows the four base bindings "
      ...
      "per §5.13. NodeIds and the example namespace are provisional.")
    A("")
    return "\n".join(L) + "\n"
...
def main():
    ...
    open(os.path.join(HERE, "OPC-UA-Facets-PubSub-Scenario-Binding-Addendum.md"), "w",
