...
**Subclauses 5.8.1 and 5.8.2 apply to inline framing and to every datagram-carried `DATA` frame.** Over `opc.quic` and the outer-protocol profile of `opc.wss3`, stream-carried `DATA` is governed by the transport's own connection- and stream-level flow control (§7.4, §8.4): no `CREDIT` frame is sent or expected for that traffic, no peer withholds stream-carried `DATA` pending one, and `Paused` is entered on QUIC, HTTP/3 or WebSocket stream or connection blocking instead. QUIC DATAGRAM and HTTP Datagram frames are congestion-controlled but are not flow-controlled by QUIC, HTTP/3 or WebSocket. Therefore datagram-carried `DATA` **shall** spend channel and connection credit exactly as this clause defines, and `CREDIT` frames for such channels **shall** be carried reliably on the channel stream or WebSocket. This keeps the resource contract deterministic; a receiver that grants no credit may drop received datagrams for that direction and **shall not** allocate unbounded state to remember them.
...
| `PartiallyReliable` | Sender-side: a droppable frame still queued at its deadline is discarded. `MaxRetransmits` has no effect, because TCP owns retransmission. | Retransmission over DATAGRAM up to `MaxRetransmits` or the deadline, then abandoned. | Retransmission over HTTP Datagrams where negotiated; otherwise sender-side discard only when §8.5 permits that degradation. |
| `Unreliable` | Sender-side discard only. Once a byte is written to the socket, TCP will deliver it. | Exact. A DATAGRAM is sent once and never retransmitted. | Exact only where HTTP Datagrams are negotiated; otherwise rejected with `Bad_DeliveryModeUnsupported`. |
...
| Item | Value |
|---|---|
| URL scheme | `opc.wss3` — provisional, pending registration by the OPC Foundation |
...
| Control and inline WebSocket sub-protocol | `opcua+uacp` |
| Data WebSocket sub-protocol | `opcua+dc` — provisional, pending registration by the OPC Foundation |
| Encoding | OPC UA Binary, as for `opc.tcp` |
...
ALPN negotiation is HTTP/3's. A Client **shall** negotiate `h3` and **shall** abandon the connection if HTTP/3 is not selected, because the stream and datagram rules of this clause do not exist on HTTP/1.1 or HTTP/2. The `opc.wss3` URL is mapped to the HTTPS target URI required by RFC 8441 and RFC 9220 by replacing the scheme with `https`, preserving the host, any non-default port, path and query, and omitting the fragment. The HTTP/3 origin is the host and port of that HTTPS target URI. The `opc.wss3` URI scheme and this HTTPS target mapping are provisional in this draft and require registration before publication.

For a normal connection, every Extended CONNECT belonging to one SecureChannel **shall** use the authority and path/query of the selected Server `EndpointUrl` after that mapping. For reverse connect, §8.7 replaces the request target with the configured Client listener authority and path/query, because the OPC UA Server is then the HTTP/3 client and is dialing the Client. In both cases the `:path` pseudo-header is the endpoint path and query, or `/` when the URL contains neither.

### 8.3 Connection establishment and the control WebSocket

The control WebSocket is established first, by an HTTP/3 Extended CONNECT request using RFC 9220. Before sending Extended CONNECT, a peer **shall** have received the peer's RFC 8441/RFC 9220 permission for Extended CONNECT, and **shall not** send Extended CONNECT otherwise. Without that explicit setting, a CONNECT stream could mean an ordinary tunnel rather than a WebSocket, and the receiver would not know which framing to apply.

The control WebSocket request **shall** carry all of the following fields:
...
| `sec-websocket-protocol` | `opcua+uacp` |

The responder **shall** return a 2xx response and **shall** echo exactly `sec-websocket-protocol = opcua+uacp` before either peer writes any UACP octet. A non-2xx response, a missing subprotocol, or a different subprotocol **shall** be treated as failure to establish the transport. The exact echo is required because `opcua+uacp` keeps its existing meaning: each WebSocket message is a complete UACP MessageChunk, and `HEL`/`ACK` follow WebSocket establishment. A data WebSocket uses `opcua+dc` instead and is not decodable by an implementation of `opcua+uacp`.
...
A data WebSocket request **shall** be an Extended CONNECT to the same `:scheme`, `:authority` and `:path` as the control WebSocket for the SecureChannel, except under reverse connect where §8.7 defines the Client listener target. It **shall** carry `sec-websocket-version = 13` and `sec-websocket-protocol = opcua+dc`. The responder **shall** return a 2xx response and echo exactly `opcua+dc` before either peer writes a data channel frame. The distinct subprotocol is required because a data WebSocket carries transport-secured data channel frames with the UA-SC symmetric security header, sequence header and footer omitted and performs no `HEL`/`ACK` handshake; reusing `opcua+uacp` would make an existing WebSocket implementation parse incompatible bytes under a registered name.

A Server **shall** validate `transportChannelId` before it binds a channel, and **shall** reject the request with `Bad_DataChannelLimitsExceeded` unless the HTTP/3 stream is open on the connection carrying the control WebSocket, was opened by the OPC UA peer required by the table, used the same authority and path as the control WebSocket as required above, carries a WebSocket negotiated with `sec-websocket-protocol = opcua+dc`, is not the control WebSocket, and is not already bound to another data channel. Under reverse connect the Client **shall** validate the `revisedTransportChannelId` returned by the Server against the same rules before it treats the channel as open. A Server **shall not** rebind a stream identifier that has been bound and released while the SecureChannel is open. A receiver that observes an unexpected identifier, or a frame on a WebSocket whose stream identifier is already bound to another channel, **shall** treat it as a protocol error on that data channel and **shall not** deliver the frame to any channel named by the frame's `ChannelId`. This is the same safety rule as §7.4: the transport binding is authoritative because `ChannelId` is redundant and attacker-controlled once the wrong stream is accepted.

A WebSocket message carries exactly one frame in QUIC framing: the Message header followed directly by the stream header, payload and nothing else. The symmetric security header, sequence header and message footer are omitted under the `TransportSecured` profile for the same reason as §7.4: HTTP/3 is QUIC, and QUIC's TLS 1.3 record layer authenticates and encrypts the bytes of every stream. Because HTTP/3 and QUIC apply their own stream and connection flow control, `CREDIT` frames **shall not** be sent to regulate WebSocket-message-carried `DATA`, and a receiver **shall** ignore such a grant unless the channel has negotiated HTTP Datagram carriage for `DATA`. For HTTP Datagram-carried `DATA`, §5.8 applies because HTTP Datagrams do not consume stream or connection flow-control credit.

`END` and `RESET` **shall** be carried only as explicit data channel frames. A WebSocket Close terminates both directions and therefore cannot express one-directional `END` on a `Bidirectional` channel, and its 16-bit close code cannot carry a 32-bit OPC UA StatusCode. A peer **shall** close the data WebSocket only after both sending directions have ended or after it has processed a `RESET` frame for that channel. A premature WebSocket close, an HTTP/3 stream reset, or an abrupt HTTP/3 stream termination that arrives before those conditions hold **shall** be treated as a transport fault on that data channel and reported as `Bad_ConnectionClosed`. A peer **shall not** encode OPC UA StatusCodes in WebSocket close codes, HTTP/3 stream error codes or QUIC application error-code fields for `opc.wss3`; RFC 9220 maps abrupt HTTP/3 stream termination to HTTP/3 errors such as `H3_REQUEST_CANCELLED`, not to arbitrary application status values.

### 8.5 Unreliable delivery

Genuine in-flight loss over `opc.wss3` requires HTTP Datagrams (RFC 9297) associated with the data WebSocket's HTTP/3 stream. HTTP Datagrams are available for a data WebSocket only when all of the following are true:

- Both peers sent `SETTINGS_H3_DATAGRAM = 1` on the HTTP/3 connection.
- QUIC transport parameters negotiated a non-zero `max_datagram_frame_size`.
- The data WebSocket Extended CONNECT request opted in with `opcua-datagram = ?1`, and the 2xx response acknowledged with `opcua-datagram = ?1`.
- The WebSocket request and response negotiated `sec-websocket-protocol = opcua+dc`.

The `opcua-datagram` field is defined by this binding as the per-request opt-in and acknowledgement that establishes RFC 9297 datagram semantics for the Extended CONNECT stream. A peer **shall not** send an HTTP Datagram for an `opc.wss3` data WebSocket unless all four conditions hold. An HTTP Datagram for the channel uses the RFC 9297 Quarter Stream ID of the data WebSocket stream as its association. The `transportChannelId` carried in `OpenDataChannel` remains the full HTTP/3 stream id, not the Quarter Stream ID.

When the negotiated delivery mode is `Unreliable`, or `PartiallyReliable` and HTTP Datagrams have been negotiated for that stream, `DATA` frames **shall** be sent as HTTP Datagrams rather than as WebSocket messages. The HTTP Datagram Payload is exactly one frame in QUIC framing. The encoded frame size **shall not** exceed `max_datagram_frame_size - sizeof_varint(stream_id / 4)`, where `stream_id / 4` is the RFC 9297 Quarter Stream ID and `sizeof_varint` is the QUIC variable-length integer encoding size. Fragmenting a frame across HTTP Datagrams is **not** permitted, for the same reason as §7.5: one lost fragment would destroy a frame that otherwise might have been useful.

HTTP Datagrams are not flow-controlled. A sender of datagram-carried `DATA` **shall** spend channel and connection credit under §5.8 before it sends, and a receiver **shall** replenish that credit on the reliable data WebSocket as it releases buffers. A receiver **shall** discard a datagram-carried `DATA` frame that exceeds the credit it has granted and **shall** `RESET` the channel with `Bad_DataChannelCreditExceeded` on the reliable data WebSocket. The DATAGRAM Capsule defined by RFC 9297 is reliable because it rides the HTTP request stream; it **shall not** be used as a fallback for genuine loss. Using a Capsule would silently turn an unreliable channel into a reliable one.

Availability and degradation are deterministic. `Unreliable` over `opc.wss3` requires genuine in-flight loss and **shall** be rejected with `Bad_DeliveryModeUnsupported` unless HTTP Datagrams are negotiated for the data WebSocket. `PartiallyReliable` may degrade to sender-side discard only when the Client selected an endpoint whose `SupportsUnreliableDatagrams` is `False`; selecting such an endpoint is the Client's indication that sender-side degradation is acceptable. If an endpoint advertises `SupportsUnreliableDatagrams` as `True` but per-stream HTTP Datagram negotiation fails, the Server **shall** reject the channel with `Bad_DeliveryModeUnsupported` rather than silently degrading it. A Server **shall not** claim genuine unreliable datagram support unless the HTTP/3 stack and configured path expose all of the mechanisms listed above.

> **Implementation note — HTTP Datagram availability.** Some HTTP/3 and WebSocket APIs expose Extended CONNECT but not HTTP Datagrams associated with the WebSocket stream, and some managed HTTP infrastructures disable datagrams even when QUIC is available. Such a stack can still conform to the non-datagram `opc.wss3` transport by advertising `SupportsUnreliableDatagrams` as `False` and using sender-side discard for `PartiallyReliable` channels; it cannot conform to the unreliable datagram conformance unit until the platform exposes RFC 9297 on the stream.
...
RFC 9114 permits HTTP/3 connection pooling and origin coalescing, but the OPC UA binding does not. A peer **shall** use a dedicated, non-pooled and non-coalesced HTTP/3 connection for each OPC UA TransportConnection, **shall** carry exactly one control WebSocket for each SecureChannel on that connection, and **shall** open every associated data WebSocket on that same HTTP/3 connection with the identical `:authority` and `:path` used by the control WebSocket. The TLS exporter value, peer certificates and HTTP/3 stream id space are connection-wide; pooling another origin or another SecureChannel onto the same connection would make the binding and stream ownership ambiguous. A stack that cannot guarantee connection pinning and prohibit coalescing **shall not** implement or advertise the outer `opc.wss3` profile and **shall** advertise and select only the inline profile if it supports `opc.wss3` at all.

0-RTT data **shall not** be used to carry Extended CONNECT, UACP octets, `OpenSecureChannel`, `OpenDataChannel` or any data channel frame. Early data is replayable, and a replayed channel open is a replayed authorization. A Server **shall** reject, and **shall not** act on, any such byte received in 0-RTT. 0-RTT **may** be used only to resume the QUIC connection before the HTTP/3 and OPC UA exchanges begin.

Certificate rotation, revocation, ticket invalidation and resumed-connection identity follow §7.6.2, except that `opc.wss3` **shall not** encode OPC UA StatusCodes in HTTP/3 or QUIC connection error-code fields. When §7.6.2 requires a connection to be closed for `Bad_SecurityChecksFailed`, an `opc.wss3` peer closes the HTTP/3 connection using protocol-appropriate error signalling and reports the OPC UA StatusCode locally and through the required audit event. A per-stream WebSocket close or HTTP/3 reset **shall not** be relied on to communicate the reason for a connection-wide security failure.

### 8.7 Reverse connect

Reverse connect over `opc.wss3` keeps the OPC UA rule of §7.10 and changes only the carrier. The **Server** opens the HTTP/3 connection to the Client's configured listener, negotiates `h3`, establishes the control WebSocket with Extended CONNECT targeted at the Client listener authority and path/query, and sends `RHE` on that WebSocket. TLS hostname validation is performed against the Client listener authority, not against a Server `EndpointUrl`. The Client replies `HEL` on the same WebSocket. The Server endpoint URL being requested travels in `RHE` and `HEL`, and endpoint selection occurs after the WebSocket connection exists; it is not the HTTP request target. Because the Server initiated the HTTP/3 connection, the TLS roles invert exactly as in §7.10: the Client is the TLS server, and §8.6 binds the Client's TLS certificate to the Client certificate in `OpenSecureChannel` while also requiring the reciprocal Server certificate check.

The data-channel mapping follows the HTTP/3 role. In a normal connection the OPC UA Client is the HTTP/3 client and therefore opens the control WebSocket and every data WebSocket with Extended CONNECT. Under reverse connect the OPC UA Server is the HTTP/3 client and performs those Extended CONNECT requests instead, all targeted at the configured Client listener authority and path/query. An implementation **shall** evaluate the table of §8.4 against the HTTP/3 role in force, because RFC 9220 gives only that role the mechanism that creates a WebSocket; the `DataChannelDirection` determines the permitted payload direction after the WebSocket exists.
...
The inline profile is optional. A Client or Server that lacks the HTTP/3 capabilities required for the outer profile **shall not** weaken those outer-profile requirements; it **shall** advertise and select only the inline profile if it supports `opc.wss3` at all, or another conforming inline transport otherwise. The outer profile requires all of the following capabilities from the HTTP/3 and TLS stack: obtaining request stream ids before the `OpenDataChannel` Service completes, inspecting incoming stream ids, pinning a SecureChannel to one non-pooled and non-coalesced HTTP/3 connection, accessing peer certificates, accessing the RFC 8446 exporter, negotiating and sending HTTP Datagrams for the datagram conformance unit, cancelling streams, and scheduling writes across the control and data WebSockets so Service traffic is not starved. A stack lacking any mandatory outer-profile capability **shall not** advertise `http://opcfoundation.org/UA-Profile/Transport/wss3-uasc-uabinary`.

A Client that cannot establish the outer `opc.wss3` profile — for example because HTTP/3 is blocked, Extended CONNECT is unavailable, policy forbids multiple WebSockets, or the platform cannot pin the connection — **may** fall back to an inline `opc.wss3` profile only by selecting an endpoint that advertises that profile. In that profile the control WebSocket is the only WebSocket, every `STR` frame is one binary WebSocket message under `opcua+uacp`, `transportChannelId` is `0` in both directions, `CREDIT` frames are used exactly as in §6, and the semantics are those of inline framing. The profile exists so an HTTP/3-only path can still carry data channels when it cannot expose one HTTP/3 stream per channel.
...
| Data Channel WebSocket HTTP/3 Transport | Data Channel Framing | Clause 8 except §8.5 and the optional inline fallback profile of §8.8: `opc.wss3`, ALPN `h3`, Extended CONNECT, one WebSocket per data channel, HTTP/3 stream-id ownership, the `TransportSecured` profile and the mandatory outer-profile capabilities. |
| Data Channel WebSocket HTTP/3 Datagram | Data Channel WebSocket HTTP/3 Transport, Data Channel Partial Reliability | §8.5: `DATA` over negotiated HTTP Datagrams with genuine in-flight loss where the platform exposes them. |
| Data Channel WebSocket HTTP/3 Inline Fallback | Data Channel Framing, Data Channel Inline Transport | The optional inline fallback profile of §8.8, advertised with `http://opcfoundation.org/UA-Profile/Transport/wss3-inline-uasc-uabinary`. |
...
| Id | Assertion | Stimulus | Expected |
|---|---|---|---|
| DCW-001 | The scheme and profile are advertised distinctly | Call `GetEndpoints` on a Server that supports the outer transport | An `opc.wss3` `EndpointUrl` is returned with `http://opcfoundation.org/UA-Profile/Transport/wss3-uasc-uabinary`, distinct from `opc.wss`, `opc.quic` and the inline fallback profile (§8.2, §8.8) |
| DCW-002 | Extended CONNECT bootstraps the control WebSocket with the required target and subprotocol | Attempt the transport without `h3`, without Extended CONNECT permission, without `:scheme = https`, with the wrong authority or path, without `sec-websocket-version = 13`, or without `sec-websocket-protocol = opcua+uacp` | The connection is abandoned before `HEL`/`ACK`; with all present and a 2xx response echoing exactly `opcua+uacp`, `HEL`/`ACK` are exchanged on the control WebSocket (§8.2, §8.3) |
| DCW-003 | One data WebSocket using `opcua+dc` is opened per data channel | Open one channel per direction | Each channel is bound to a distinct HTTP/3 stream id carried in `transportChannelId` or `revisedTransportChannelId`; every data WebSocket negotiates `opcua+dc`, not `opcua+uacp` (§8.4) |
...
| DCW-005 | Datagram support is negotiated, associated and reported honestly | Open a datagram channel with `SETTINGS_H3_DATAGRAM = 1`, non-zero `max_datagram_frame_size` and `opcua-datagram = ?1`, then repeat with each missing | Success uses HTTP Datagrams associated by the RFC 9297 Quarter Stream ID while `transportChannelId` remains the full stream id; missing negotiation makes `SupportsUnreliableDatagrams` `False` or rejects genuine-loss requests with `Bad_DeliveryModeUnsupported` (§8.5) |
...
| DCW-008 | Reverse connect targets the Client listener, not the Server endpoint | Establish by reverse connect and inspect the control and data Extended CONNECT requests | The requests target the configured Client listener authority and path; TLS hostname validation uses that authority; the Server endpoint travels in `RHE`/`HEL` and is selected after the connection exists (§8.7) |
| DCW-009 | Inline fallback is optional and not a confidentiality downgrade | Block outer-profile data WebSockets and offer no inline fallback, then offer an inline fallback endpoint whose SecurityMode is `Sign` | Without an advertised inline endpoint the Client reports no conforming fallback; with `Sign` only it also reports failure because inline `Sign` leaves payload in the clear (§8.8) |
| DCW-010 | All directions are opened by the HTTP/3 client | Open `SourceToSink`, `SinkToSource` and `Bidirectional` channels under normal and reverse connect | Normal connection: the OPC UA Client supplies and the Server echoes the stream id. Reverse connect: the OPC UA Server opens each data WebSocket and returns the validated id in `revisedTransportChannelId` (§8.4, §8.7) |
| DCW-011 | The reverse-connect stream id returned by the Server is validated | Establish by reverse connect and return a `revisedTransportChannelId` for a stream that is absent, uses the control WebSocket, uses `opcua+uacp`, or was opened on another HTTP/3 connection | The Client refuses the channel and does not deliver frames from that stream (§8.4, §8.7) |
| DCW-012 | The Server checks the binding too, not only the Client | Connect over `opc.wss3` presenting a TLS client certificate whose key differs from the certificate in the `OpenSecureChannel` request | The Server aborts the SecureChannel before any `OpenDataChannel` succeeds, and raises `AuditSecurityEventType` (§8.6, mirroring DCQ-021) |
| DCW-013 | A data channel is refused without an authenticated TLS client | Complete an HTTP/3 TLS connection without presenting a TLS client certificate, then call `OpenDataChannel` | `Bad_SecurityChecksFailed` (§8.6, mirroring DCQ-022) |
| DCW-014 | The reciprocal binding holds under reverse connect | Establish by reverse connect and present, as the OPC UA Server, a TLS client certificate whose key differs from the certificate in the `OpenSecureChannel` response | The Client, acting as TLS server, aborts the SecureChannel and raises `AuditSecurityEventType` (§8.6, §8.7, mirroring DCQ-026) |
| DCW-015 | Reverse connect refuses an unauthenticated TLS client | Complete a reverse-connect HTTP/3 TLS handshake without presenting a TLS client certificate, then attempt `OpenDataChannel` | `Bad_SecurityChecksFailed` (§8.6, §8.7, mirroring DCQ-027) |
| DCW-016 | 0-RTT carries no OPC UA or WebSocket-opening bytes | Attempt Extended CONNECT, `OpenSecureChannel`, `OpenDataChannel` or a data channel frame in early data | The bytes are rejected and not acted on (§8.6) |
| DCW-017 | HTTP/3 pooling and coalescing are prohibited | Attempt to share one HTTP/3 connection across origins or SecureChannels, or open a data WebSocket with a different authority or path | The outer profile is not advertised or the connection/channel is refused; each SecureChannel uses a dedicated non-coalesced connection (§8.4, §8.6) |
| DCW-018 | WebSocket close is not used as half-close or StatusCode carriage | Send `END` in one direction of a `Bidirectional` channel, then close the WebSocket before the peer's direction ends; separately send `RESET` as a frame | The one-way `END` leaves the WebSocket open; premature close is reported as `Bad_ConnectionClosed`; `RESET` status is taken only from the explicit frame (§8.4) |
| DCW-019 | HTTP Datagrams spend OPC UA credit | Grant `N` bytes on a datagram channel, then send more than `N` bytes of datagram-carried `DATA` | Excess is discarded and the channel is reset with `Bad_DataChannelCreditExceeded`; no unbounded receive state is allocated (§5.8, §8.5) |
| DCW-020 | A DATAGRAM Capsule is not accepted as unreliable delivery | Negotiate an unreliable channel, then carry `DATA` in a DATAGRAM Capsule on the request stream | The frame is not treated as genuine-loss delivery; the channel is rejected or reset according to the malformed transport use (§8.5) |
...
| Clause 8 | New `7.8 OPC UA WebSocket over HTTP/3` | The complete new `opc.wss3` transport: ALPN `h3`, RFC 9220 Extended CONNECT, one WebSocket per data channel on its own HTTP/3 stream using `opcua+dc`, optional HTTP Datagrams, the inherited TLS peer binding, reverse connect and the inline fallback profile. The editor may choose the final clause number if the QUIC insertion is numbered differently. The TransportProfileUri values are registered in OPC 10000-7. |
