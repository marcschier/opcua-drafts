...
- **The layers shall also be bound cryptographically wherever the TLS stack permits it, and the binding shall be negotiated so it cannot be stripped.** Where the SecurityPolicy in force supports the enhanced SecureChannel signature of OPC 10000-6 §6.7.5 and both peers' TLS implementations expose an RFC 8446 §7.5 exporter, the Client **shall** set the `TransportBinding` flag in the `OpenSecureChannel` request and include the exporter value, taken with the label `EXPORTER-opcua-quic` and an empty context, in that `ChannelThumbprint` computation; a Server that supports it **shall** echo the flag, do likewise, and reject the request with `Bad_SecurityChecksFailed` where the value does not verify. A Server **shall not** clear the flag once a Client has set it, and a Client **shall** abort the SecureChannel if a Server it has previously observed to support the binding fails to echo it. The flag travels inside the asymmetrically secured `OpenSecureChannel` body and is therefore not removable by a relay — without that, a peer that mixes in the exporter and one that does not simply compute different signatures and fail to interoperate, which is indistinguishable from an attack and gives an implementer every reason to turn the binding off. Where either condition does not hold the binding is unavailable, and the peer **shall** record that through `AuditSecurityEventType`, because the deployment is then protected by the certificate comparison alone.
...
- **The binding shall be mutual, because the peer whose content is at risk is the one that must check.** A Server **shall** request TLS client authentication on every `opc.quic` connection, and **shall not** accept `OpenDataChannel` on a SecureChannel whose QUIC connection completed without an authenticated TLS client certificate, returning `Bad_SecurityChecksFailed`. The key-equality obligation above then applies symmetrically and unconditionally: the certificate presented by the peer acting as **TLS client shall** carry the same `subjectPublicKeyInfo` as the certificate that peer presents in the `OpenSecureChannel` exchange, and the peer acting as TLS server **shall** abort the connection unless it does. Neither peer **shall** rely on its counterpart having performed the reciprocal check. Without this the protection is unilateral and lands on the wrong side: only the TLS client compares, so a relay that terminates TLS toward the Client and connects onward to the Server as an unauthenticated TLS client is invisible to the Server, which has no check to perform and no signal that one was skipped — and since the comparison fails open, a single Client that never implements it exposes that Server's content with the Server unable to prevent, detect or audit it. A peer that aborts a connection for a key-equality failure **shall** generate an `AuditSecurityEventType` Event naming both certificates.
...
> **Implementation note — exporter availability.** At the time of writing, the .NET QUIC implementation exposes no RFC 8446 §7.5 exporter on its TLS connection, so a stack built on it can satisfy only the certificate comparison. This is recorded for the same reason as the datagram gap of §7.5: an obligation a major platform cannot meet should be visible in the document rather than discovered during certification. It is also why the comparison, and its negative test DCQ-007, are mandatory rather than advisory — on such a platform they are the whole of the defence.

> **Implementation note — session-ticket invalidation.** Likewise, .NET offers no way to invalidate outstanding QUIC session tickets at a chosen moment, so the resumption rule of §7.6.2 cannot be met by invalidation there; a stack on that platform disables resumption for `opc.quic` instead, which the clause explicitly permits. Three obligations in this document now have that character — RFC 9221 datagrams, the RFC 8446 exporter, and ticket invalidation — and in each case the specification states the requirement and names the conforming fallback rather than weakening the rule to whatever one platform happens to offer.
...
- **The obligation follows the TLS role, and the artifacts differ by direction.** Under a normal connection the peer acting as TLS server is the OPC UA Server, and the binding is between its TLS certificate, the `serverCertificate` of the selected `EndpointDescription`, and the Server certificate in the `OpenSecureChannel` **response**. Under reverse connect (§7.10) the OPC UA Client acts as TLS server, and the mandatory TLS-server binding is then between the Client's TLS certificate and the Client certificate in the `OpenSecureChannel` **request**; an `EndpointDescription` describes a Server endpoint, so no Client equivalent exists to compare against and that pair is the whole of the TLS-server obligation. The Client's separate duty to satisfy itself of the Server's identity is unchanged and is carried by the `OpenSecureChannel` exchange, and by TLS client authentication where the deployment uses it — in which case the symmetric obligation applies to the Server's certificate too. Every rule in this subclause is to be read against whichever pair applies.
