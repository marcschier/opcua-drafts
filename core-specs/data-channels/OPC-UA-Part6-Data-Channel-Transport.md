...
> **Implementation note — WebTransport status.** WebTransport over HTTP/3 is, at the time of writing, `draft-ietf-webtrans-http3` draft-16, an active Standards Track Internet-Draft and not an RFC. Every other transport named by this document rests on stable RFCs. A stack that advertises `opc.wt` **shall** implement the draft version profiled here, and a product whose platform cannot provide a conforming WebTransport API **shall not** advertise this transport. At the time of writing .NET exposes no WebTransport API, so the reference implementation cannot implement `opc.wt`; this is recorded for the same reason §7.6.1 records platform limits around QUIC datagrams, TLS exporters and ticket invalidation.
