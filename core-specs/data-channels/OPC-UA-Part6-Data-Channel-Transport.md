...
- **The layers shall also be bound cryptographically wherever the TLS stack permits it.** Where both peers' TLS implementations expose an RFC 8446 §7.5 exporter and the SecurityPolicy in force is not `None`, the Client and Server **shall** include the exporter value, taken with the label `EXPORTER-opcua-quic` and an empty context, in the `ChannelThumbprint` computation of OPC 10000-6 §6.7.5. A peer that cannot obtain an exporter **shall** proceed on the certificate comparison above alone, and **shall** record that it did so, so that a deployment can tell which of the two bindings is actually protecting it. This obligation is stated separately from the comparison because the two fail in opposite directions, which is the single most important property of this subclause: the certificate comparison is procedural and **fails open**, since a Client that simply never implements it interoperates perfectly with every honest Server and misbehaves only in the presence of a relay, whereas the exporter binding is cryptographic and **fails closed**, since a relay terminating TLS necessarily derives a different exporter and cannot produce a `ChannelThumbprint` the peer will accept, whether or not the implementer ever reasoned about relays. Neither replaces the other: the exporter binding does not prove the TLS peer is the OPC UA application, and the comparison does not survive an implementer who omits it.
- **Where either peer requires TLS authentication of the other**, the same key-equality obligation applies symmetrically to that peer's TLS certificate.
...
- **A verifying peer shall not substitute a cached certificate for the comparison, nor launder a mismatch as a rotation.** The comparison **shall** be repeated on every new connection against the artifacts of that connection rather than against any retained from an earlier one. Discovery is the one place the comparison legitimately spans two connections, since an `EndpointDescription` obtained earlier may have been superseded by the time the Client connects; a Client that finds the mismatch **should** re-read the endpoints once and retry before reporting a security failure. That retry **shall** abandon the failed connection rather than re-reading over it, **shall** preserve the `ApplicationUri`, host, `EndpointUrl`, transport profile, SecurityMode and SecurityPolicy of the endpoint it originally selected, and **shall not** override an explicitly configured certificate pin. Re-reading over the failed connection would let the party that caused the mismatch supply the replacement anchor, which is precisely the relay §7.6.1 rejects.
...
| DCQ-016 | The discovery retry cannot be used to launder a mismatch | Configure a certificate pin on the Client, present a key-unequal certificate, and offer a fresh `GetEndpoints` response over the failed connection | The Client abandons the failed connection rather than re-reading over it, preserves the originally selected endpoint parameters, and reports a security failure rather than accepting the pinned key's replacement (§7.6.2) |
