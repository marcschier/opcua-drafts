...
| transportChannelId | UInt64 | Over an outer-protocol transport, the id of a transport carrier the Client has already opened and will not write to until the response arrives. Over `opc.quic` this is the QUIC stream id for a Client-opened direction (`SinkToSource` or `Bidirectional`) and `0` for `SourceToSink`. Over outer-profile `opc.wss3` this is the HTTP/3 stream id of the data WebSocket for every direction on a normal connection; under reverse connect the OPC UA Server is the HTTP/3 client, so the Client sends `0` for every direction. Always `0` over inline framing. See the Part 6 errata §7.4 and §8.4. |
...
| revisedTransportChannelId | UInt64 | The underlying transport identifier in force: the QUIC stream id over `opc.quic`, the HTTP/3 stream id of the data WebSocket over outer-profile `opc.wss3`, `0` for inline framing. When the Client supplied the id in `transportChannelId`, this value is echoed unchanged after validation. When the Server opened the carrier — `SourceToSink` over `opc.quic`, and every direction over reverse-connect `opc.wss3` — this value carries the Server-selected id. It is **not** named `transportChannelId`: OPC UA Service definitions do not reuse a parameter name across a request and its response, and the standard model compiler enforces that by validating both against one field-name table, so a Service carrying `transportChannelId` in both is rejected outright. |
...
**Preconditions.** The Session **shall** be activated. The SecureChannel **shall** be one over which the transport supports data channels; `Bad_DataChannelTransportUnsupported` is returned otherwise. Opening the channel **shall not** take the count over `MaxDataChannels` for the connection or `MaxChannels` for the source. Over `opc.quic`, a request for a Client-opened direction **shall** carry `transportChannelId`. Over outer-profile `opc.wss3`, a normal-connection request **shall** carry `transportChannelId` for every direction, and a reverse-connect request **shall** carry `0` for every direction. The peer that did not open the outer-protocol carrier **shall** validate the id before accepting the binding: normally the Server validates a request id, but under reverse-connect `opc.wss3` the Client validates `revisedTransportChannelId` in the response.
...
A Server **shall not** transmit any frame for the assigned ChannelId before the response carrying that ChannelId has been handed to the transport, and a Client **shall** buffer rather than reject a frame naming a ChannelId whose `OpenDataChannel` is still outstanding. Over outer-protocol transports the response and the frames travel on different QUIC or HTTP/3 streams, which are not ordered relative to each other, so without this rule the first `DATA` frame can legitimately overtake the response and be rejected as an unknown ChannelId. The Part 6 errata §7.4 and §8.4 state the transport-level form of the same obligation.
...
A reduction of `MaxFrameSize` cannot take effect instantaneously at both ends, because `ModifyDataChannel` is a Service: the revised parameters reach only the caller, there is no `MODIFY` frame type, and over outer-protocol transports the response and the frames are not ordered relative to each other. A Client that applied a smaller limit the moment the response arrived would see legitimately in-flight larger frames and, under the Part 6 errata §5.12, kill the channel. Therefore:
