...
Because that binding ties several artifacts together, **replacing a key moves them all at once**, and a peer that moves them independently breaks the binding with no attacker present. A connection's binding is fixed when it is established, and a peer never presents two different keys on one connection, including at token renewal. A renewal that re-issues the *same* key is therefore transparent — the binding is by key, so nothing need be disturbed. A change of key is not: TLS 1.3 fixes the certificate at handshake and cannot renegotiate, so activating a new key closes the connections bound to the old one, with a QUIC `CONNECTION_CLOSE` carrying `Bad_SecurityChecksFailed` rather than per-stream resets, which are not guaranteed to survive the close. A verifier closes on learning its peer's bound certificate is revoked, untrusted or expired rather than trusting the holder to do it, and session resumption inherits the identity of the handshake that issued the ticket, so tickets are invalidated on activation. This is an easy set of rules to violate silently, because the natural implementation resolves the TLS certificate once when the listener starts while the certificate registry rotates underneath it, and the resulting mismatch looks to the verifying peer exactly like the relay attack the binding exists to prevent. Whether a *key* change may be staged, so that long-lived media survives one, is left as an open question for the Working Group, because granting it touches OPC 10000-12 and OPC 10000-4 rather than Part 6 alone.
...
| A TLS-terminating proxy sits in the path | **Inline** (`opc.tcp`, `opc.wss`) | `TransportSecured` gives such an intermediary the plaintext, and no QUIC profile avoids that (§9.5); inline framing secures every frame end to end. |
