...
```powershell
python source\cloud-specs\schema-registry\tools\build_model.py
python source\cloud-specs\schema-registry\tools\validate_local.py
```

The generated NodeSet resolves the released xRegistry model from `model/dependencies/`. Draft numeric NodeIds use the provisional `62000+` block in `http://opcfoundation.org/UA/SchemaRegistry/` (namespace index 2 in the NodeSet, after the xRegistry base at index 1); final NodeIds are assigned by the OPC Foundation.
