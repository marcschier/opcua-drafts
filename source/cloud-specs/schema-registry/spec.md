...
### Downloading a schema (mandatory) {#sec-downloading-a-schema-mandatory}
...
### Registering a schema (optional) {#sec-registering-a-schema-optional}
...
## Schema-specific information model {#sec-schema-specific-information-model}
...
| **Attribute** | **Value** |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
| BrowseName | 2:SchemaRegistryType |  |  |  |  |
| IsAbstract | False |  |  |  |  |
...
### SchemaFileType {#sec-schemafiletype}
...
| SREG-SchemaDownload |  |  |  |  |  |
| SREG-Identity |  |  |  |  |  |
| SREG-SchemaVersioning |  |  |  |  |  |
| SREG-TtlMirror |  |  |  |  |  |

### SchemaId-NodeId fast access and GetSchema {#sec-schemaid-nodeid-fast-access-and-getschema}
...
### Formats and content-types {#sec-formats-and-content-types}
...
### Schema identity (`SchemaId`) and per-format fingerprints {#sec-schema-identity-schemaid-and-per-format-fingerprints}
...
### Group and schema identifiers {#sec-group-and-schema-identifiers}
...
## Schema versioning and compatibility {#sec-schema-versioning-and-compatibility}
...
## Resolution flow {#sec-resolution-flow}
...
## TTL and mirror semantics {#sec-ttl-and-mirror-semantics}
...
## Structure materialization and the xRegistry API {#sec-structure-materialization-and-the-xregistry-api}

### Auto-bootstrap of the schema structure {#sec-auto-bootstrap-of-the-schema-structure}
...
## Optional PubSub integration {#sec-optional-pubsub-integration}
...
## PubSub DataSet schema profile (normative for the profile, optional) {#anx-c annex=normative}
...
### C.1 ConfigurationVersion-keyed evolution {#sec-c-1-configurationversion-keyed-evolution}
...
### C.2 Open unions subject to aggregation {#sec-c-2-open-unions-subject-to-aggregation}
...
### C.3 Append-only rule (normative) {#sec-c-3-append-only-rule-normative}
...
### C.4 PubSub integration touch-points {#sec-c-4-pubsub-integration-touch-points}
...
### C.5 Example (informative) {#sec-c-5-example-informative}
...
An executable reference demonstration of append-only Variant and ExtensionObject growth — latest-minor-decodes-older, distinct per-minor SchemaIds, and opaque-body fallbacks appended append-only at a non-reserved ordinal — is provided at `extras/core-specs/avro-encoding/tools/evolution_demo.py`.
