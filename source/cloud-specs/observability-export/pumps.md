...
## Observability export bindings for `PumpType` {#sec-observability-export-bindings-for-pumptype}

Bindings for `PumpType` in `http://opcfoundation.org/UA/Pumps/`, per the [Observability Export](spec.md) base specification. Each binding exposes one OTEL signal (`Metrics`, `Logs` or `Traces`) with a deterministic `DataSetClassId`.
