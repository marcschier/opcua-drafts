...
## Observability export bindings for `MotionDeviceSystemType` {#sec-observability-export-bindings-for-motiondevicesystemtype}

Bindings for `MotionDeviceSystemType` in `http://opcfoundation.org/UA/Robotics/`, per the [Observability Export](spec.md) base specification. Each binding exposes one OTEL signal (`Metrics`, `Logs` or `Traces`) with a deterministic `DataSetClassId`.
...
### Traces — Traces {#sec-traces-traces}
...
## BrowsePath resolution — worked examples {#sec-browsepath-resolution-worked-examples}
...
### Topology 1: Single 6-axis articulated robot {#sec-topology-1-single-6-axis-articulated-robot}
...
### Topology 2: Single 4-axis SCARA {#sec-topology-2-single-4-axis-scara}
...
### Topology 3: Two-robot cell (6+4 axes) {#sec-topology-3-two-robot-cell-6-4-axes}
