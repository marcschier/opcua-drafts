...
- Full specification: [OPC-UA-Observability-Export.md](spec.md)
...
- **Pumps** — metrics on `PumpType`: [addendum](pumps.md).
- **Robotics** — `MotionDeviceSystemType` with multi-device/axis cardinality expansion: [addendum](robotics.md).
- **DI** — `IVendorNameplateType` and **DeviceHealth** (`IDeviceHealthType`): [DI](di.md) · [DeviceHealth](dihealth.md).
- **Facets** — binding inheritance and facet composition across subtype, interface and AddIn: [addendum](facets.md).
...
**Status — working draft.** For Working Group discussion: scope, Part number and namespace, and the conformance model. Full specification → [OPC-UA-Observability-Export.md](spec.md).
