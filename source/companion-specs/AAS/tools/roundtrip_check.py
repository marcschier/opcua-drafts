...
MODEL = os.path.abspath(os.path.join(
    HERE, os.pardir, os.pardir, os.pardir, os.pardir, "model", "companion-specs", "AAS"))
FIXTURES = os.path.join(HERE, "fixtures")
NODESET = os.path.join(MODEL, "Opc.Ua.I4AAS.NodeSet2.xml")
NODEIDS = os.path.join(MODEL, "Opc.Ua.I4AAS.NodeIds.csv")
...
MODEL_VERSION = "3.00-draft4"
MODEL_PUBLICATION_DATE = "2026-08-31T00:00:00Z"
...
TEMPLATE_CLAUSE_ANCHORS = {
    "6.5.4": "sec-versioning-and-the-lifecycle-record",
    "6.5.5": "sec-discovery-and-resolution",
    "6.5.6": "sec-federation-x",
    "6.5.7": "sec-information-disclosure-tiers",
    "10": "sec-profiles-and-conformance-units",
}
...
def _markdown_clause(text, number):
    ...
    anchor = TEMPLATE_CLAUSE_ANCHORS.get(number)
    if not anchor:
        return ""
    match = re.search(
        rf"^(#+)\s+.*\{{#{re.escape(anchor)}(?:\s[^}}]*)?\}}\s*$",
        text,
        re.MULTILINE,
    )
    if match is None:
...
def _validate_security_integrity_contracts(root, aliases, spec_text):
    ...
    conformance_clause = _markdown_clause(spec_text, "10")
    _require_fragments(
