...
### Where Arrow fits {#sec-where-arrow-fits}
...
### Columnar model, IPC framing and endianness {#sec-columnar-model-ipc-framing-and-endianness}
...
### Schema identity and reversibility {#sec-schema-identity-and-reversibility}
...
## OPC UA Arrow DataEncoding {#sec-opc-ua-arrow-dataencoding}

### OPC UA Arrow {#sec-opc-ua-arrow}
...
#### General rules {#sec-general-rules}
...
#### Built-in DataTypes {#sec-built-in-datatypes}

For generated per-type schemas and examples, see [Annex A](#anx-a).
...
#### Enumerations and OptionSets {#sec-enumerations-and-optionsets}
...
#### Structures, optional fields and Unions {#sec-structures-optional-fields-and-unions}
...
#### ExtensionObject and abstract/subtyped fields {#sec-extensionobject-and-abstract-subtyped-fields}
...
#### Default Arrow DataTypeEncoding Node {#sec-default-arrow-datatypeencoding-node}
...
#### Schema-generation algorithm {#sec-schema-generation-algorithm}
...
An encoder is not handed a foreign schema to encode against. Encoding a value **produces** its Arrow Schema deterministically from the inputs above: the OPC UA DataTypeDefinition of the declared DataType and, for Variant values and for fields declared abstract or allowing subtypes, the concrete runtime type of the value. When a schema is **grown** rather than created from scratch, the **previous schema of the lineage is also an input**: the generator appends the newly encountered Variant body form or ExtensionObject concrete type to that prior schema as an additional dense-union child, leaving every existing child and type code unchanged (§5.6.12). When **no previous schema is supplied, generation starts a fresh lineage — a reset that begins a new MajorVersion**. A **decoder** that re-derives a schema uses the identical inputs, so both sides agree on every field, child and type code.
...
#### Decoder algorithm {#sec-decoder-algorithm}
...
In the primary use case the decoder — a subscriber or downstream Arrow consumer — **likely does not have** the OPC UA type model or DataTypeDefinitions at consumption time. The type model may live elsewhere but is unavailable where the batch is decoded, so that decoder cannot re-derive the schema. It instead **obtains the schema by SchemaId**, from the embedded Schema message of an IPC stream, a cache, an announcement or a schema registry. Only a decoder that does hold the relevant DataTypeDefinitions uses the AddressSpace-driven path above; the SchemaId is then sufficient to verify that both sides are using the same schema.
...
#### Implementing schema evolution (growing unions) {#sec-implementing-schema-evolution-growing-unions}
...
4. **Recompute the SchemaId, publish and version.** Each grown union yields a new serialized Arrow `Schema`. Recompute the SHA-256[:8] SchemaId per §5.6.9. The publisher **shall** advance the DataSet `ConfigurationVersion` MinorVersion, and **shall publish the new schema to subscribers — registering it in the Schema Registry and announcing it** per the Part 14 Arrow handshake (§6.9.2.3, §6.9.2.5) — before sending a batch that uses the appended child.
...
## PubSub message mapping {#sec-pubsub-message-mapping}
...
### Arrow mapping parameters {#sec-arrow-mapping-parameters}
...
### Arrow message mapping {#sec-arrow-message-mapping}
...
### Bare RecordBatch framing (`batch`) {#sec-bare-recordbatch-framing-batch}
...
### Content types {#sec-content-types}
...
### Part 14 configuration model mapping {#sec-part-14-configuration-model-mapping}
...
### Header layouts {#sec-header-layouts}
...
### Content-type registry entries {#sec-content-type-registry-entries}
...
### DataSet schema mapping {#sec-dataset-schema-mapping}
...
### NetworkMessage and DataSetMessage envelopes {#sec-networkmessage-and-datasetmessage-envelopes}
...
#### Schema resolution {#sec-schema-resolution}
...
#### SchemaId handshake {#sec-schemaid-handshake}

##### SchemaId and canonical schema bytes {#sec-schemaid-and-canonical-schema-bytes}
...
Because an Arrow NetworkMessage is **one homogeneous RecordBatch whose Arrow Schema is that DataSet's schema**, the per-DataSet SchemaId is **inherent to the columnar model**: there is no envelope that could aggregate several DataSets' fields into one schema. Arrow therefore does **not** adopt the fixed NetworkMessage envelope with opaque per-DataSet payload entries that the Avro mapping defines (Avro Part 14 §8.1) — that construct exists to keep a *row* envelope's schema stable while it carries several DataSets, a problem the columnar model does not have. An implementer shall not introduce such an envelope into the Arrow mapping; a frame that would carry several DataSets is expressed as several streams or batches, each with its own SchemaId.
...
##### Carrier placement {#sec-carrier-placement}
...
##### Schema announcements {#sec-schema-announcements}
...
An encoder shall maintain `announced: set[SchemaId]` per destination. Before sending a batch, it recomputes the Arrow Schema from the DataSet fields using the Part 6 algorithm and computes the 8-byte SchemaId. If the SchemaId has not been announced to that destination, the encoder publishes it first and then adds it to `announced`.

**Publishing a changed schema is one rule with two channels.** On a new SchemaId the publisher **shall register the schema in the Schema Registry and announce it to subscribers** — the announcement by opening a new IPC stream or otherwise sending the IPC Schema message or `ArrowSchemaAnnouncement`, and the registration through the Schema Registry write lifecycle (see *OPC UA — Schema Registry* §5.2). A deployment without a reachable registry announces only, and records that the registry publish is unavailable. The publisher **shall** also advance the DataSet `ConfigurationVersion` (§5.6.12 step 4, §6.9.2.7); decoders remain SchemaId-authoritative.
...
##### Decoder cache-miss resolution {#sec-decoder-cache-miss-resolution}
...
3. Resolve the schema from an external (federated) xRegistry that the local registry references, as defined by [*OPC UA for Schema Registry*](https://github.com/OPCF-Members/spec-drafts/blob/main/source/cloud-specs/schema-registry/spec.md) §8.
4. Read the in-server AddressSpace Schema Registry by a SchemaId-NodeId. The Schema Registry companion model uses namespace `http://opcfoundation.org/UA/SchemaRegistry/` and exposes each schema at an Opaque NodeId whose Identifier is the raw 8-byte SchemaId. A decoder may perform a single `Read` on that NodeId without browsing or recomputing candidate NodeIds. Servers may additionally expose `GetSchema(SchemaId)` for clients that prefer a Method call over direct NodeId construction.
...
##### Relationship to ConfigurationVersion {#sec-relationship-to-configurationversion}
...
Although SchemaId is not computed from ConfigurationVersion, the `{MajorVersion, MinorVersion}` lineage carries a compatibility relationship between successive schemas per *OPC UA — Schema Registry* §5.6. A MinorVersion increment is an append-only-compatible superset of the same major. A MajorVersion increment is a reset with no such guarantee.

An Arrow IPC `stream` embeds its writer schema and remains self-describing. A bare RecordBatch carries no schema, and its node and buffer layout matches its writer-minor schema, so a subscriber shall resolve the exact writer-minor schema by SchemaId to decode it, or apply a defined upgrade that treats union children absent from the older RecordBatch as empty.

A publisher **shall** advance MinorVersion when it aggregates a new Variant body form or ExtensionObject concrete type into the schema, and shall publish the resulting schema — registry registration plus announcement — per §6.9.2.3–6.9.2.6. The per-encoding procedure for narrowing and append-only growing these unions is in the Arrow Part 6 DataEncoding §5.6.12.

##### Schema-exchange sequences {#sec-schema-exchange-sequences}
...
## Historian and ADBC access {#sec-historian-and-adbc-access}
...
### ADBC-style statement model {#sec-adbc-style-statement-model}
...
### Canonical result shape {#sec-canonical-result-shape}
...
### Raw, modified, and at-time reads {#sec-raw-modified-and-at-time-reads}
...
### Processed aggregate reads {#sec-processed-aggregate-reads}
...
### Content types and transport {#sec-content-types-and-transport}
...
### Relationship to Part 14 PubSub {#sec-relationship-to-part-14-pubsub}
...
### Built-in SByte {#sec-arrow-reference-built-in-sbyte}
...
### Built-in Byte {#sec-arrow-reference-built-in-byte}
...
### Built-in Int16 {#sec-arrow-reference-built-in-int16}
...
### Built-in UInt16 {#sec-arrow-reference-built-in-uint16}
...
### Built-in Int32 {#sec-arrow-reference-built-in-int32}
...
### Built-in UInt32 {#sec-arrow-reference-built-in-uint32}
...
### Built-in Int64 {#sec-arrow-reference-built-in-int64}
...
### Built-in UInt64 {#sec-arrow-reference-built-in-uint64}
...
### Built-in Float {#sec-arrow-reference-built-in-float}
...
### Built-in Double {#sec-arrow-reference-built-in-double}
...
### Built-in String {#sec-arrow-reference-built-in-string}
...
### Built-in DateTime {#sec-arrow-reference-built-in-datetime}
...
### Built-in Guid {#sec-arrow-reference-built-in-guid}
...
### Built-in ByteString {#sec-arrow-reference-built-in-bytestring}
...
### Built-in XmlElement {#sec-arrow-reference-built-in-xmlelement}
...
### Built-in NodeId {#sec-arrow-reference-built-in-nodeid}
...
### Built-in ExpandedNodeId {#sec-arrow-reference-built-in-expandednodeid}
...
### Built-in StatusCode {#sec-arrow-reference-built-in-statuscode}
...
### Built-in QualifiedName {#sec-arrow-reference-built-in-qualifiedname}
...
### Built-in LocalizedText {#sec-arrow-reference-built-in-localizedtext}
...
### Built-in ExtensionObject {#sec-arrow-reference-built-in-extensionobject}
...
### Built-in DataValue {#sec-arrow-reference-built-in-datavalue}
...
### Built-in Variant {#sec-arrow-reference-built-in-variant}
...
### Built-in DiagnosticInfo {#sec-arrow-reference-built-in-diagnosticinfo}
...
### Composite Array<String> {#sec-arrow-reference-composite-array-string}
...
### Composite Matrix<Double> {#sec-arrow-reference-composite-matrix-double}
