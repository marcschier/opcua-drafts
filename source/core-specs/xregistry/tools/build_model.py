...
# OPC 10000-5 5.2.4: a Server publishes the version and publication date of every namespace
# it hosts as a NamespaceMetadataType Object under Server/Namespaces. Without one a Client -- and
...
#
# These Nodes are appended last so that adding them cannot renumber anything above.
...
def namespace_metadata(uri, version, pubdate, is_subset=False):
    ...
    meta = _mid()
    # The BrowseName is the namespace URI in this model's own namespace, and the Properties
    # under it are the base ones, so they keep their namespace-0 BrowseNames.
    ...
    def _prop(name, datatype, xml_type, text):
        ...
        add(nid, "UAVariable", name, f"NamespaceMetadata_{name}", parent=T(meta),
            attrs={"DataType": datatype, "ValueRank": "-1", "_ns0bn": True})
        ref(nid, HasTypeDefinition, PropertyType)
    ...
    _prop("NamespaceUri", String, "String", uri)
    _prop("NamespaceVersion", String, "String", version)
    _prop("NamespacePublicationDate", DateTime, "DateTime", pubdate)
    _prop("IsNamespaceSubset", Boolean, "Boolean", "true" if is_subset else "false")
    return meta
