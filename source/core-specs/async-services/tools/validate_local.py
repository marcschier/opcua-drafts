...
MODEL = os.path.abspath(os.path.join(
    HERE, os.pardir, os.pardir, os.pardir, os.pardir, "model", "core-specs", "async-services"))
REF = os.path.join(HERE, "ref")
NS = "{http://opcfoundation.org/UA/2011/03/UANodeSet.xsd}"

XML = os.path.join(MODEL, "Opc.Ua.AsyncServices.NodeSet2.xml")
CSVF = os.path.join(MODEL, "Opc.Ua.AsyncServices.NodeIds.csv")
...
if not combined_services:
    combined_services = _slice(
        annex_free.get(COMBINED, ""),
...
_fig_tools = os.path.abspath(os.path.join(
    HERE, "..", "..", "..", "..", "word-drafts", "tools"))
