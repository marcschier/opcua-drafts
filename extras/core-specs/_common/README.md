...
Infrastructure shared by the encoding extensions (`avro-encoding/`, `arrow-encoding/`).
...
| `json_control.py` | A type-directed **reference codec** (JSON) that round-trips the whole corpus. Not the OPC UA JSON DataEncoding — it is the template the Avro/Arrow codecs follow, and the control that proves the corpus + equality are sound. |
...
Each encoding extension implements `encode`/`decode` against its wire library (fastavro / pyarrow) and runs the shared `CORPUS` through it.
...
```powershell
python extras/core-specs/_common/validate_local.py     # 100 cases, 0 failures
```
