...
One NamespaceUri `http://opcfoundation.org/UA/WoT-Con/` at model version `1.3.0` (namespace index 2 in the NodeSet, after Core at 0 and the xRegistry base at 1). The incorporated 1.02 nodes keep their published NodeIds `1..172`; the additive registry nodes use a provisional **64000+** block (types) with members allocated append-only from **64500**. Final registry NodeIds are assigned by the OPC Foundation.
