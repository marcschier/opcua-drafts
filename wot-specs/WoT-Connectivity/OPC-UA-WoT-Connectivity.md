...
- **Affordances** map to nodes as above; **links** map to OPC UA **References** (`uav:componentModel`/`uav:typedReference` → `HasComponent`/typed references, `uav:reference`/`uav:capability` → non-hierarchical references, `uav:componentOf` → the parent `HasComponent` of §7.3) or, when no native reference fits, to an explicit link representation carried on the projected node.
