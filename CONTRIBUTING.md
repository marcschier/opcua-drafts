...
- The same rule covers the two rendered artifacts. The Word documents under [`word-drafts/`](word-drafts/) are rebuilt from the specifications, and the slide deck under [`decks/`](decks/) is rebuilt from `decks/content/*.yaml` with `python decks/build_deck.py`. Never hand-edit a `.docx` or the `.pptx` — the edit is lost on the next build.
