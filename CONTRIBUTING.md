...
- Each `source/<group>/<extension>/` folder holds the specification and any tools it owns directly. Secondary tooling, descriptors, validators and examples live under `extras/<group>/<extension>/`. Group aggregates live at `extras/<group>/validate_all.py`. **Each group drives only its own validators**, so a specification that changes groups takes its aggregate entry with it.
...
python extras/core-specs/<extension>/tools/validate_local.py
```
...
conventions, and how examples and validators are built — is documented in
[`.github/copilot-instructions.md`](.github/copilot-instructions.md), alongside the build and
validation commands and a map of the repository. Read it before drafting or editing spec text or a
model.
