...
| **AAS Registry — xRegistry mapping** | Not a specification but the study behind one: how the Asset Administration Shell maps onto xRegistry, and what the product passport standards require of that mapping. It exists because the AAS registry/repository split turns out to be xRegistry's document-versus-URL distinction, and because an AAS records no history at all while a passport must be retrievable as it stood on a date — so the registry supplies the one thing the metamodel cannot. The specifications it produced were proposed to the xRegistry project, not the OPC Foundation. | Study 0.1.0 | [Study](cloud-specs/aas-registry/AAS-xRegistry-Study.md) · [Folder](cloud-specs/aas-registry/README.md) |
